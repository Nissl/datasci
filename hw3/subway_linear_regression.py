...
def predictions(dataframe):
    ...
    print cost_history
    # r_sq_top = np.sum(np.square(predictions - values))
    # print r_sq_top
    # r_sq_bottom = np.sum(np.square(values - np.mean(values)))
    # print r_sq_bottom
    # r_squared = 1 - (np.sum(np.square(predictions - values)) / np.sum(np.square(values - np.mean(values))))
    # print r_squared

    return predictions

local_dir = "/Users/johnmorgan/datasci/hw3"
input_file = "%s/turnstile_data_master_with_weather.csv" % local_dir
turnstile_weather = pandas.read_csv(input_file)
